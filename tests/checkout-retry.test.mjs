...
function harness() {
  ...
  const dependencies = { 'next/server': { NextResponse: Response }, '@/lib/supabase/server': { createClient: async () => ({ auth: { getUser: async () => ({ data: { user: { id: user } } }) } }) }, '@/lib/supabase-admin': { supabaseAdmin: db }, '@/lib/stripe': { stripe }, '@/lib/email/reservation-confirmed': { dispatchReservationConfirmedEmail: async () => {} }, '@/lib/email/table-confirmed': { dispatchTableConfirmedEmails: async () => {} } };
